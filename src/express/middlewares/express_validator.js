...
export const create_user_body_validator = [
...
export const update_user_body_validator = [
    body("username").optional().trim().isLength({min: 5}).withMessage(
        "The username must be at least 5 characters long"
    ),
    body("email").optional().trim().isEmail().withMessage(
        "This is not a valid email"
    ),
    body("password").optional().trim().isLength({min: 8}).withMessage(
        "The password must be at least 8 characters long"
    )
]

export const user_id_param_validator = param("user_id").custom(user_id => {
